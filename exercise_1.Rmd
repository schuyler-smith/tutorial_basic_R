...
2-3 4
```
It is looking for an operation after the 3 and finds only whitespace before a new numeric. Because R reads from left to right. Alternatively, if you give an operation and don't follow it up with an argument then R will wait for you to complete it:
...
```
Note that this function does not return the number as a numeric and R does not have intuitive data-type handling, but it can be told how to
```R
...
runif()
```
We get an error.`argument "n" is missing, with no default`. What does this mean.. every function in R or a package for R has a hlp page and can be accessed by typing `?function` into the console.
```R
?runif
```
Under __Arguments__ we see that `n` us the number of times the function generate a random number. So if we correct the error and give a value to n.
```R
runif(n=2)
```
...
!x
```
Gives the binary opposite.
* And `&`
```R
y <- c(FALSE,TRUE,1,0)
x&y
```
Checks binary values of each element-pair, returns TRUE if both are TRUE.
* Logical-and `&&`
```R
x <- c(TRUE,1)
y <- c(TRUE,0)
...
x&y
```
Checks if all elements are TRUE in both objects.
* Or `|`
```R
y <- c(TRUE,0)
x|y
...
x|y
```
Checks if each element-pair has at least one TRUE value.
* Logical-or `||`
```R
x <- c(FALSE,0)
y <- c(FALSE,0)
...
x||y
```
Checks if either object has at least one TRUE value.
...
melt()
```
`Error in melt() : could not find function "melt"` The session has no function assigned to `melt()`. Now let's install the package that defines what `melt()` is.
```R
